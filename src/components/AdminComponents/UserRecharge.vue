<template>
	<div>
		<el-form class="change-package">
			...
			<el-form-item label="">
				<el-input type="text" v-model="amount" placeholder="充值金额" autocomplete="off"></el-input>
			</el-form-item>
			<el-form-item>
	...
	methods: {
		fetchData() {
			...
			if (this.username != "" && this.amount != 0) {
				axios.get('/api/recharge?username=' + this.username + '&amount=' + this.amount).then(response => {
				...
				ElMessage.error("用户名不能为空且金额不能为零");
			}
		}
	}
}
